...
"""@package checker
The checker to validate a reduction
"""
...
def local_check(formula_old, formula_new, certificate_old, certificate_new_local, variables_original, variables_requested):
    ...
    if not consistent_formula(formula_old, formula_new):
        eprint(f"\n\tError! New formula {formula_new} is not consistent with original!")
        return False

    if not consistent_certificate(certificate_old, certificate_new_local, formula_new):
        eprint(f"\n\tError! New certificate {certificate_new_local} is not consistent with original")
        return False

    if not consistent_variables(variables_original, variables_requested, formula_new):
        eprint(f"\n\tError! New variables used {variables_original, variables_requested} were not requested!")
        return False
